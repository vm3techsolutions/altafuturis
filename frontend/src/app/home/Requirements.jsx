...
const QuoteSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json(); // JSON parse done safely
      ...
      if (response.ok) {
        ...
        setResponseMessage(data.message || "Submission failed. Try again.");
      }
    ...
    <div className="flex flex-col md:flex-row lg:flex-row items-center justify-between bg-[#6328A6] text-white p-10 lg:p-20 gap-10">
      ...
      <div className="lg:w-1/2 md:w-1/2 w-full text-center lg:text-left">
        <div className="md:w-full w-full flex justify-center md:justify-start mb-16 md:mb-0">
          <h4 className="px-5 py-3 text-xl border-2 border-brownColor rounded-full text-white bg-[#6328A6] shadow-sm font-bold">
      ...
      <div className="bg-white text-black p-8 rounded-lg shadow-lg w-[60vw] lg:w-1/3 md:w-1/2">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="bg-[#6328A6] text-white py-3 rounded-lg hover:bg-brownColor transition-all"
          >
