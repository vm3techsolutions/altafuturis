      ...
      try {
        ...
        const formatted = json.categories.flatMap((cat) =>
          cat.caseStudy.map((item) => ({
            ...
            slug: item.link.replace(/\//g, "").toLowerCase(), // generate slug from link
          }))
        );

        // Optional: take only first 3 items like your original layout
        setCaseStudies(formatted.slice(0, 3));
      } catch (err) {
