...
export default function BlogCaseStudiesComponent() {
  ...
  //       setCategories(
  //         json.categories.map((cat) => ({ id: cat.category_name, name: cat.category_name }))
  //       );
  //       setSelectedBlog(allBlogs[0]);
  //     });
  // }, []);
  ...
  const handleCategoryFilter = (category) => {
    ...
    if (category === null) {
      ...
      const filtered = data.filter((b) => b.categoryId === category);
        ...
        <div className="w-full md:w-3/5 p-4 space-y-10">
          {selectedBlog && (
            <div key={selectedBlog.id} className="overflow-hidden md:p-5">
              ...
              <div className="p-4 space-y-5 border-l-2 border-[#D4B301] -mt-2">
                <h2 className="text-xs mb-2">📅 {selectedBlog.p}</h2>
                <h2 className="text-3xl font-semibold">{selectedBlog.title}</h2>

                <p className="text-gray-600 mt-2 text-lg line-clamp-6">
          ...
          {displayedBlogs
            ...
            .map((blog) => (
              ...
              >
                ...
                <Link
  href={`/blog/${blog.slug}`}
  className="text-[#6328A6] text-sm inline-block font-semibold hover:text-brownColor"
>
  VIEW BLOG »
</Link>
