...
const CaseStudyLayout = () => {
  ...
  <button
    ...
    className={`w-full sm:w-auto px-4 py-2 border rounded-xl transition-colors duration-300 ${
      filteredCategory === null ? "bg-blueColor text-white" : "border-gray-300 hover:bg-brownColor hover:text-white"
    }`}
  >
    All Categories
  </button>
  {categories.map((category) => (
    <button
      ...
      className={`w-full sm:w-auto px-4 py-2 border rounded-xl transition-colors duration-300 ${
        filteredCategory === category.id ? "bg-blueColor text-white" : "border-gray-300 hover:bg-brownColor hover:text-white"
      }`}
    >
      {category.name}
    </button>
        ...
        <div className="w-full md:w-3/5 p-4 space-y-10">
          {displayedCaseStudies.slice(0, 1).map((study) => (
            <div key={study.id} className="overflow-hidden md:p-5">
              <img className="w-full h-80 object-cover rounded-lg" src={study.image} alt={study.title} />
        ...
        <div className="w-full md:w-2/5 p-4 overflow-y-auto max-h-[800px]">
          {displayedCaseStudies.slice(1).map((study) => (
            <div key={study.id} className="mb-4 overflow-hidden md:p-5">
              <img className="w-full h-30 object-cover rounded-lg border-l-2 border-[#D4B301]" src={study.image} alt={study.title} />
